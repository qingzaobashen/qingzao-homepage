...
## 二、Flexbox弹性容器：现代布局的主力

Flexbox弹性容器：使用`display: flex`或`display: inline-flex`将一个元素声明为Flexbox，它的直接子元素自动成为flex弹性元素。弹性容器+弹性子元素构成了一个弹性盒子。

弹性子元素在弹性容器中呈一字排列，从左到右。

容器属性如：
- flex-direction: 指定弹性子元素在父容器中的位置。它可以是：
	`row`（从左到右横排左对齐，-默认）、`row-reverse`（从右到左，右对齐）、`column`（纵向排列，上对齐）或`column-reverse`（纵向反转）。
- align-items：子元素在父容器中的纵向位置：
	`flex-start`（顶对齐）、`flex-end`（底对齐）、`center`（垂直居中）
- flex-wrap：子元素是否换行：`nowrap`（不换行，默认）、`wrap`（换行，按子元素宽度）
